...
import { ArrowLeft, Users, Package, BarChart3, FileText, Download, Upload, Trash2, Edit, Plus } from "lucide-react"
...
export default function AdminInterface({
  ...
  const clearEmployeeBalance = (id: string) => {
    ...
    const label = roundedBalance > 0
      ? `Schulden von €${roundedBalance.toFixed(2)}`
      : `Guthaben von €${Math.abs(roundedBalance).toFixed(2)}`

    if (confirm(`Bezahlung für ${employee.name} verbuchen? ${label}`)) {
      ...
      // Alle bisherigen Transaktionen dieses Mitarbeiters löschen, nur Bezahlung behalten
      const otherTransactions = transactions.filter((t) => t.employeeId !== id)
      onUpdateTransactions([...otherTransactions, paymentTransaction])
    }
  }
